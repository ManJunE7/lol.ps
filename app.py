...
# 유틸
def metric_card(label, value, delta=None, help=None):
...
with left:
    ...
    # 룬
    rune = row.get("best_rune", "")
    st.markdown(f"**추천 룬**: {rune if isinstance(rune, str) and rune.strip() else '—'}")
    # 스펠
    bsc = row.get("best_spell_combo", row.get("best_spells", ""))
    st.markdown(f"**추천 스펠**: {bsc if isinstance(bsc, str) and bsc.strip() else '—'}")
    # 시작/신발/코어
    bstart = row.get("best_start", "")
    bboots = row.get("best_boots", "")
    bcore3 = row.get("best_core3", "")
    st.markdown(f"**시작 아이템**: {bstart if isinstance(bstart, str) and bstart.strip() else '—'}")
    st.markdown(f"**2티어 신발**: {bboots if isinstance(bboots, str) and bboots.strip() else '—'}")
    st.markdown(f"**코어 3코어**: {bcore3 if isinstance(bcore3, str) and bcore3.strip() else '—'}")
